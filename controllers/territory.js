...
export const getListOfAvailableTerritories = (req, res, next) => {
  ...
  Territory.find({ $and: [{ congregation: req.user._id }, { type: "free" }] })
    .exec()
    .then((territories) => {
      Territory.paginate(
...
export const searchAllTerritories = (req, res, next) => {
  ...
  const congregationID = req.user.username
    ? req.user._id
    : req.user.congregation;
  if (typeof req.query.city !== "undefined") {
    const regex = new RegExp(escapeRegex(req.query.city), "gi");
    Territory.paginate(
      {
        $and: [{ city: regex }, { congregation: congregationID }],
      },
      paginationOptions,
    )
      .then((result) => {
        Preacher.find({ congregation: congregationID })
          .sort({ name: 1 })
          .exec()
          .then((preachers) => {
            res.json(result);
    ...
    Territory.paginate(
      {
        $and: [{ street: regex }, { congregation: congregationID }],
      },
      paginationOptions,
    )
      .then((result) => {
    ...
    Territory.paginate(
      {
        $and: [{ number: req.query.number }, { congregation: congregationID }],
      },
      paginationOptions,
    )
      .then((result) => {
    ...
    Preacher.findOne({
      $and: [{ congregation: congregationID }, { _id: req.query.preacher }],
    })
      .exec()
      .then((preacher) => {
        Territory.paginate(
    ...
    Territory.paginate(
      {
        $and: [{ kind: req.query.kind }, { congregation: req.user._id }],
      },
      paginationOptions,
    )
      .then((result) => {
    ...
    Territory.paginate(
      {
        $and: [{ type: { $not: /free/ } }, { congregation: req.user._id }],
      },
      paginationOptions,
    )
      .then((result) => {
...
export const getTerritoryHistory = (req, res, next) => {
  Territory.findById(req.params.territory_id)
    .populate([
      "preacher",
      "history",
      {
    ...
    .then((territory) => {
      Territory.find({ congregation: req.user._id })
        .exec()
        .then((territories) => {
          const currentIndex = territories.findIndex(
            (t) => t._id.toString() === territory._id.toString(),
          );
...
export const assignTerritory = (req, res, next) => {
  Territory.findById(req.params.territory_id)
    ...
    .then((territory) => {
      const taken =
        req.query.isDateChosen === "true"
          ? new Date(req.body.taken).toISOString().slice(0, 10)
...
export const deleteTerritory = (req, res, next) => {
  Territory.findByIdAndDelete(req.params.territory_id)
    .exec()
